...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    // Join two datasets
    spark.udf.register("ST_Contains",(queryRectangle:String, pointString:String)=>(HotzoneUtils.stContains(queryRectangle, pointString)))
    ...
    // groupBy rectangle values 
    val countDF = joinDf.groupBy("rectangle").count()
    // sort them after coalesce
    val outputDF = countDF.sort("rectangle").coalesce(1)
    return outputDF 
  }
